...
namespace ArnoldCinterpreter {
    ...
    public class Lexer {
        ...
        public Lexer() {
	        ...
	        var lexeme_dict = new Dictionary<int, Tuple<string, string>>();
            ...
            // 		var item_tuple = item.Value;
            // 		// var remove_key = item.Key + 1;
            // 		var identifier_str = item_tuple.Item1;
            // 		if (identifier_str == "Variable identifier") {
            // 			lexeme_dict.Remove(item.Key);
            // 		}
           	// 	}
            ...
            // check if symbol table's complete
            foreach (KeyValuePair<int, Tuple<string, string>> item in lexeme_dict) {
